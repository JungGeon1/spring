...
<div id="applyBox">
<button onclick="cansleApply()" class="btn btn-default" id="cansleApply">x</button>
<form id="apply">
  <table>
       <tr>
      ...
      <tr><td><input type="submit" value="신청" class="btn btn-default"></td></tr>
  </table>
</form>
</div>

<div id="editBox">
<button onclick="cansleEdit()" class="btn btn-default" id="cansleEdit">x</button>
<form id="edit">
  <table>
  	<tr>
      ...
      <tr><td><input type="submit" value="수정" class="btn btn-default"></td></tr>
  </table>
</form>
</div>
	...
	<div class="searchBox">
		...
		<form id="searchList">
			<h3 id="sTitle" class="whiteColor">검색</h3>
...
function showList() {
	...
	function del(idx) {
		 if(confirm('입양 안 하시게여?...인성..?')){
		           $.ajax({
	                url : 'http://localhost:8080/abandonedDog/rest/'+idx+'?id='+'<%=session.getAttribute("id")%>',
	                type : 'DELETE',
	                dataType:'text',
	                success : function(data){
		...
		$.ajax({
			...
			success : function(data) {
				...
				if($(val).find('desertionNo').text()==dno){
					...
					html+='<br>';
					html+='<h3 class="whiteColor">신청된 정보입니다</h3>';
					html+='<br>';
				...
				alert(error);
				   ...
				   if(data.length==0){
	                ...
	                for(var i=0; i<data.length;i++){
	                    ...
	                    html += '신청제목 : ' + data[i].a_title +'\n';
	                    html += '신청내용 : ' + data[i].a_text +'<br>\n';
	                    html += '<button class="btn btn-default" onclick="edit('+ data[i].a_idx +')">수정하기</button>\n';
	                    html += '<button class="btn btn-default" onclick="del('+ data[i].a_idx +')">삭제하기</button>\n';
	              	    html += '</div>\n';
	                }
